...
<script type="text/javascript">
    function acheter(num) {
        ...
        location.href = "billet_changer.html";
    }
</script>
<meta charset='utf-8' />
</head>
...
<?php
	...
	$nomgare_depart = $_COOKIE['gare_depart'];
	$nomgare_arrivee = $_COOKIE['gare_arrivee'];
	$heure = $_POST['heure'];
	$date = $_POST['date'];
	...
	include "connect.php";
	$vConn = fConnect();
	$vSql = "SELECT distinct gare_depart.numero, type_train.typ, type_train.numero as numtrain, gare_depart.day as jour, gare_depart.horrairepassage as horaire_depart,'$date' as date_dep, gare_depart.ville as ville_depart,gare_depart.nom as gare_depart, gare_depart.cp as cp_depart,  '$date' as date_arrivee,gare_arrivee.horrairepassage as horaire_arrive,gare_arrivee.ville as ville_arrivee ,gare_arrivee.nom as gare_arrivee, gare_arrivee.cp as cp_arrivee from (select * from dessert D  where horrairepassage > '$heure' and day = '$day' AND nom = '$nomgare_depart') as gare_depart, (select * from dessert D where horrairepassage > '$heure' and day = '$day' AND  nom = '$nomgare_arrivee') as gare_arrivee,(select * from trains ) as type_train,(select * from itineraire_horaire) as horaire where (gare_depart.numero = gare_arrivee.numero) AND (horaire.numitineraire = gare_depart.numero) AND (gare_depart.day = gare_arrivee.day) AND (gare_arrivee.horrairepassage > gare_depart.horrairepassage) AND (horaire.numtrain = type_train.numero) AND (horaire.jour = gare_depart.day) and (horaire.heure = gare_depart.horrairepassage or horaire.heure = gare_arrivee.horrairepassage)";
			...
			<?php
				...
				while($vResult=pg_fetch_array($vQuery)){
					if(isset($vResult[0])){
						...
						echo "<td ><button type='button' onclick = 'acheter($i);' class='button'>Changer</button></td>";
						echo "</tr><br/>";
						}
						else{
...
if($vQuery){
					//echo "success!<br/>";
					$page_retour = "changer_date_billet.html";
